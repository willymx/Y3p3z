...
// Code Highlignt and Copy ( Clipboard ) and Components nav Link Active
class Code {
...
class ThemeLayout {
     ...
     constructor() {
          ...
          this.mainNavWrapper = document.getElementsByClassName('main-nav')[0];
          this.mainNavWrapperContent = document.getElementsByClassName('main-nav')[0].innerHTML;
          this.mainNavScrollbarContent = this.mainNavScrollbar.outerHTML;
          this.mainNavScrollbarInnerContent = this.mainNavScrollbar.innerHTML;
     }
     ...
     activateMenu() {
          ...
          var activatedItem;
          let pageUrl = windowUrl;
          if (windowUrl.split("/")[windowUrl.split("/").length - 1] === "") {
               pageUrl = windowUrl + "index.html";
          }
          document.querySelectorAll('.main-nav .navbar-nav a')
               .forEach(function (element) {
                    if (!shouldBreak) {
                         const target = element;
                         activatedItem = element
                         if (element.href === pageUrl) {
                              target.classList.add('active');
                              let parentId = target.getAttribute('data-target-toggle');
                              while (parentId != null) {
                                   if (parentId === 'sidebarMore' && !self.reverseQuery(target, '#sidebarMore')) {
                                        break;
                                   }
                                   let parentElement = document.querySelector('a[href="#' + parentId + '"]');
                                   if (parentElement && parentElement.classList) {
                                        let parentCollapse = parentElement.parentElement.querySelector('.collapse');
                                        parentElement.classList.add('active');
                                        parentId = parentElement.getAttribute('data-target-toggle');
                                        if (self.html.getAttribute("data-layout") === "vertical" && parentCollapse)
     ...
     initVerticalMenu() {
          this.activateMenu();
          // Auto Close Menu
          document.querySelectorAll('.main-nav .nav-link').forEach(function (element) {
               element.addEventListener('click', function (e) {
                    document.querySelectorAll('.main-nav .nav-link').forEach(function (newElement) {
                         if (newElement !== element) {
                              bootstrap.Collapse.getInstance(document.getElementById(newElement.getAttribute('aria-controls')))?.hide();
                         }
                    });
               });
          });
          ...
          document.querySelectorAll('.main-nav .sub-nav-link.menu-arrow').forEach(function (element) {
               element.addEventListener('click', function (e) {
                    document.querySelectorAll('.main-nav .sub-nav-link.menu-arrow').forEach(function (newElement) {
                         if (newElement !== element) {
                              bootstrap.Collapse.getInstance(document.getElementById(newElement.getAttribute('aria-controls')))?.hide();
                         }
                    });
     ...
     initScrollBars() {
          ...
          if (html.getAttribute("data-layout") === "vertical") {
               document.querySelector('.main-nav .scrollbar')?.setAttribute("data-simplebar", "");
               document.querySelector('.main-nav .scrollbar')?.classList.add('h-100');
          }
     }
     ...
     initHorizontalLayout() {
          if (html.getAttribute("data-layout") === "horizontal" && window.innerWidth > 1040) {
               this.splitHorizontalMenu();
               this.activateMenu(true);
          }
     }

     // Split Horizontal Layout Menu Setup
     splitHorizontalMenu() {
          const splitMenu = 6; // After this number all horizontal menus will be moved in more menu options
          const extraMenuName = 'More';
          this.mainNavWrapper.innerHTML = this.mainNavScrollbarInnerContent;
          ...
          menuData.forEach(function (item, index) {
               ...
               if (index + 1 === menuData.length) {
                    splitItem.insertAdjacentHTML("afterend", '<li class="nav-item sidebar-more">\
                    <a class="nav-link menu-link menu-arrow" href="#sidebarMore" data-bs-toggle="collapse" role="button"\
                         aria-expanded="false" aria-controls="sidebarMore">\
                         <span class="nav-icon"><i class="bx bx-briefcase-alt-2"></i></span>\
                         <span class="nav-text"> ' + extraMenuName + ' </span>\
                    </a>\
                    <div class="collapse menu-dropdown" id="sidebarMore">\
                    <ul class="nav sub-navbar-nav">' + newMenus + '</ul></div></li>');
               }
          });
     }

     // Vertical Layout Menu Setup
     initVerticalLayout() {
          this.mainNavWrapper.innerHTML = this.mainNavWrapperContent;
          document.getElementsByClassName('scrollbar')[0]?.setAttribute("data-simplebar", "");
          document.getElementsByClassName('scrollbar')[0]?.classList.add('h-100');
          new SimpleBar(document.getElementsByClassName('scrollbar')[0]);
     }
     ...
     changeThemeLayout(layout, save = true && window.innerWidth >= 1040) {
          this.html.setAttribute('data-layout', layout);
          if (save) {
     ...
     resetTheme() {
          ...
          this.changeThemeLayout(this.config.layout);
     ...
     initSwitchListener() {
          ...
          document.querySelectorAll('input[name=data-layout]').forEach(function (element) {
               element.addEventListener('change', function (e) {
                    self.changeThemeLayout(element.value);
               })
          });
               ...
               element.addEventListener('click', function () {
                    ...
                    if (configSize === 'sm-hover-active') {
                         if (size === 'sm-hover-active') {
                              self.changeMenuSize('sm-hover', false);
                         } else {
                              self.changeMenuSize('sm-hover-active', false);
                         }
                    }
               });
          })

          if (this.html.getAttribute("data-layout") === "vertical") {
               var container = document.querySelector('.swipe-open');
               if (container !== null) {
     ...
     setSwitchFromConfig() {
          ...
          document.querySelectorAll('.right-bar input[type=radio]').forEach(function (checkbox) {
               checkbox.checked = false;
          })

          var config = this.config;
          if (config) {
               var layoutNavSwitch = document.querySelector('input[type=radio][name=data-layout][value=' + config.layout + ']');
